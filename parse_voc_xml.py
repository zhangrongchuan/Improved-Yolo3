...
names_dict = {}
cnt = 0
f = open('./data/classes.txt', 'r').readlines()
for line in f:
    line = line.strip()
    names_dict[line] = cnt
    cnt += 1
...
#test_cnt = 0
def gen_test_txt(path_,txt_path):
    global test_cnt
    f = open(txt_path, 'w')

    for i, path in enumerate(path_):
...
#train_cnt = 0
def gen_train_txt(path_,txt_path):
    global train_cnt
    f = open(txt_path, 'w')
...
gen_test_txt(val_path,'test.txt')
